...
| 4    |  실행 및 디버그 | 재생버튼과 벌레모양 | 프로그램을 실행하고 오류를 찾아보는 역할을 한다.          |
...
따라서 대소문자 구분을 킨 것으로 검색하는 것이 grep에서 검색한 것 같다고 생각된다.
...
나는 terminal로 commit하는 것보다 소스 제어 화면에서 commit하는 것이 더욱 편했다.
terminal은 실행 후에 따로 commit을 하지만 소스 제어 화면에서 하는 경우 파일의 변경 사항을 눈으로 확인하면서 commit할 수 있기 때문에 더욱 보기쉽고 편리하다고 느꼈다.
